...
shared class RespawnSystem
{
	...
	CBlob@ SpawnPlayerIntoWorld(Vec2f at, PlayerInfo@ p_info)
	{
		...
		if (player !is null)
		{
			string blobName = p_info.blob_name;
			if (blobName != "fighter" && blobName != "interceptor" && blobName != "bomber" && blobName != "scout")
			{
				blobName = "fighter";
			}
			CBlob @newBlob = server_CreateBlob(blobName, p_info.team, at);
