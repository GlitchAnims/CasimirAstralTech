...
shared class RulesCore
{
	...
	void AddPlayerSpawn(CPlayer@ player)
	{
		...
		if (lastBlobName.length() > 0 && p !is null)
		{
			if (lastBlobName != "fighter" && lastBlobName != "interceptor" && lastBlobName != "bomber" && lastBlobName != "scout")
			{
				lastBlobName = "fighter";
			}
			p.blob_name = filterBlobNameToSpawn(lastBlobName, player);
		}
