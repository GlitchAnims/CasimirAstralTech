...
void onTick(CBlob@ this)
{
	...
	if (hitWall) //if there was no hit, but there is a wall, move bullet there and die
	{
		...
		if (is_client)
		{
			Sound::Play("dig_dirt2.ogg", futurePos, 1.5f + (0.2f * shot_r.NextFloat()), 1.0f + (0.2f * shot_r.NextFloat()));
		}
		this.server_Die();
	}
}
...
void doMuzzleFlash(Vec2f thisPos = Vec2f_zero, Vec2f flashVec = Vec2f_zero)
{
	...
	for(int i = 0; i < particleNum; i++)
   	{
		u8 alpha = 40 + (170.0f * shot_r.NextFloat()); //randomize alpha
    	...
    	if(p !is null)
    	{
			...
			p.bounce = 0;
			p.Z = 8;
			p.timeout = 2.0f + (6.0f * shot_r.NextFloat());
		}
	}
